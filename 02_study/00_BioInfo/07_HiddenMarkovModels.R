...
nucleotides <- c("A", "C", "G", "T") # Define the alphabet of nucleotides
afterAprobs <- c(0.2, 0.3, 0.3, 0.2) # Set the values of the probabilities, where the previous 
afterCprobs <- c(0.1, 0.41, 0.39, 0.1) # Set the values of the probabilities, where the previous 
afterGprobs <- c(0.25, 0.25, 0.25, 0.25) # Set the values of the probabilities, where the previous 
afterTprobs <- c(0.5, 0.17, 0.17, 0.17) # Set the values of the probabilities, where the previous 
...
ATrichprobs <- c(0.7, 0.3) # Set the probabilities of switching states, where
GCrichprobs <- c(0.1, 0.9) # Set the probabilities of switching states, where
...
generatehmmseq <- function(transitionmatrix, emissionmatrix, initialprobs, seqlength)
{
  ...
  mysequence[1] <- firstnucleotide # Store the nucleotide for the first position of the 
  mystates[1] <- firststate # Store the state that the first position in the sequence 
  for (i in 2:seqlength)
  {
    prevstate <- mystates[i-1] # Get the state that the previous nucleotide in the # Get the probabilities of the current state, given that the previous nucleotide was generated 
    ...
    nucleotide <- sample(nucleotides, 1, rep=TRUE, prob=probabilities)
    mysequence[i] <- nucleotide # Store the nucleotide for the current position of 
    mystates[i] <- state # Store the state that the current position in the 
  }
...
makeViterbimat <- function(sequence, transitionmatrix, emissionmatrix)
{
  ...
  for (i in 2:length(sequence)) # For each position in the DNA sequence:
  {
    ...
    {
      # Find the probabilility, if we are in state l, of choosing the nucleotide at position 
      statelprobnucleotidei <- emissionmatrix[l,sequence[i]]
      
      # v[(i-1),] gives the values of v for the (i-1)th row of v, ie. the (i-1)th position in 
      # In v[(i-1),] there are values of v at the (i-1)th row of the sequence for each possible 
      # v[(i-1),k] gives the value of v at the (i-1)th row of the sequence for a particular state 

      # transitionmatrix[l,] gives the values in the lth row of the transition matrix, xx should 
