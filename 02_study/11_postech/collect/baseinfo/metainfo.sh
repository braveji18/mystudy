...
LOG_FILE="${WORK_DIR}/log/metainfo.${NOW}.log"

mkdir -p ${WORK_DIR}/log/

cd $WORK_DIR

./metainfo_work.sh  2>&1 | tee -a  $LOG_FILE 

if [ -f "${WORK_DIR}/_SUCCESS_META_INFO" ] ; then
    echo "CMD OK!!"
else
    echo "send error mail !!"
    python  ../send_mail.py  "메타정보 업로드 에러"  $LOG_FILE    
    exit -1
fi
